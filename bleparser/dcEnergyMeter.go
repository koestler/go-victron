...
type DcEnergyMeterRecord struct {
	...
	AuxMode        victronDefinitions.DcEnergyMeterAuxMode `Description:"Aux mode"`
...
func DecodeDcEnergyMeterRecord(inp []byte) (ret DcEnergyMeterRecord, err error) {
	...
	ret.AuxMode = victronDefinitions.DcEnergyMeterAuxMode(inp[8] & 0x3)
	switch ret.AuxMode {
	case victronDefinitions.DcEnergyMeterAuxModeAuxVoltage:
		if v := binary.LittleEndian.Uint16(inp[6:8]); v != 0x7FFF {
			ret.AuxVoltage = float64(int16(v)) / 100
		} else {
			ret.AuxVoltage = math.NaN()
		}
	case victronDefinitions.DcEnergyMeterAuxModeTemperature:
