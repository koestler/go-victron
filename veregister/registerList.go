...
func NewRegisterList() RegisterList {
	return RegisterList{
		...
		FieldListRegisters: make([]FieldListRegisterStruct, 0),
	}
}

// Len returns the number of registers in the list.
func (rl *RegisterList) Len() int {
	return len(rl.NumberRegisters) + len(rl.TextRegisters) + len(rl.EnumRegisters) + len(rl.FieldListRegisters)
}
...
func (rl *RegisterList) AppendFieldListRegisterStruct(r ...FieldListRegisterStruct) {
	rl.FieldListRegisters = append(rl.FieldListRegisters, r...)
}
...
func (rl *RegisterList) FilterRegister(f func(r Register) bool) {
	...
	rl.FieldListRegisters = filterRegisters(rl.FieldListRegisters, f)
}
